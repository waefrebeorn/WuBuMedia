...
REM           (3) the perceive->think->speak loop (conversational)
REM           (4) the secure browser bridge (WS :18765)
...
if errorlevel 1 (
  ...
  cd /d %ROOT%
  start "wubu-face" cmd /k "python -m http.server 8137 --bind 127.0.0.1 --directory %ROOT%\face"
) else (
...
if errorlevel 1 (
  ...
  cd /d %ROOT%
  start "wubu-bridge" cmd /k "python browser\wubu_bridge.py"
) else (
