...
def main():
    ...
    # RVC's vc_infer_pipeline.py applies median_filter(f0, 3) before coarse:
    #   from rvc.lib.tools.pr import median_filter  -> scipy.signal.medfilt(f0, 3)
    # (zeros participate in the window, so unvoiced frames stay 0). Our C11
    # engine matches this exactly — WITHOUT it this reference is NOT a true
    # RVC baseline (the 2026-08-09 regression hunt proved the missing median
    # was the audible phoneme regression).
